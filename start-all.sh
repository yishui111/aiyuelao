...
echo "[4/8] 启动 ANL 前端（端口 5176）..."
(cd "$ROOT/projects/ANL" && npm run dev > "$LOGS/anl-frontend.log" 2>&1 &)

echo "[5/8] 启动 AI 匹配引擎 ai-powered-matching-algorithm（端口 8014）..."
(cd "$ROOT/projects/ai-powered-matching-algorithm" && ./.venv/Scripts/python.exe -m uvicorn main:app --host 127.0.0.1 --port 8014 > "$LOGS/matching-algo.log" 2>&1 &)

echo "[6/8] 启动 GlowMeet 后端（内存持久化, 端口 8013）+ 前端（端口 3000）..."
(cd "$ROOT/projects/GlowMeet/backend" && PERSISTENCE=memory PORT=8013 X_CLIENT_ID=dev-client-id X_CLIENT_SECRET=dev-client-secret X_REDIRECT_URL=http://localhost:3000/auth/x/callback APP_JWT_SECRET=dev-secret ./glowmeet.exe > "$LOGS/glowmeet-backend.log" 2>&1 &)
(cd "$ROOT/projects/GlowMeet/web" && npm run dev > "$LOGS/glowmeet-web.log" 2>&1 &)

echo "[7/8] 启动 Shidduch 后端（端口 8015）+ 前端（端口 5174）..."
(cd "$ROOT/projects/shidduch-app/backend" && ./.venv/Scripts/python.exe -m uvicorn app.main:app --host 127.0.0.1 --port 8015 > "$LOGS/shidduch-backend.log" 2>&1 &)
...
check "http://localhost:5176/"            "ANL 前端        " "$LOGS/anl-frontend.log"
check "http://127.0.0.1:8014/docs"        "AI匹配引擎      " "$LOGS/matching-algo.log"
check "http://localhost:8013/"            "GlowMeet 后端   " "$LOGS/glowmeet-backend.log"
check "http://localhost:3000/"            "GlowMeet 前端   " "$LOGS/glowmeet-web.log"
check "http://127.0.0.1:8015/health/ready""Shidduch 后端   " "$LOGS/shidduch-backend.log"
