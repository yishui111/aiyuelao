...
if (-not (Test-Path $KS)) {
    & "$JDK\bin\keytool.exe" -genkeypair -keystore $KS -alias weiliao `
        -storepass weiliao123 -keypass weiliao123 -keyalg RSA -keysize 2048 -validity 10000 `
        -dname "CN=WeiLiao,O=WeLiao,C=CN"
}
& "$BT\apksigner.bat" sign --ks $KS --ks-pass pass:weiliao123 --key-pass pass:weiliao123 `
    --out "$ROOT\wechat-app\wechat-release.apk" "$BUILD\aligned.apk"
if ($LASTEXITCODE -ne 0) { throw "apksigner 失败" }

# ---- 8. 校验 ----
Step 8 "校验签名..."
& "$BT\apksigner.bat" verify --print-certs "$ROOT\wechat-app\wechat-release.apk" | Select-Object -First 2

$size = [math]::Round((Get-Item "$ROOT\wechat-app\wechat-release.apk").Length / 1KB)
Write-Host ""
Write-Host "构建完成 ✓  D:\xm\aiyuelao\wechat-app\wechat-release.apk ($size KB)"
Write-Host "安装：把 APK 传到手机安装；或 USB 连接后  tools\android-sdk\platform-tools\adb.exe install -r wechat-release.apk"
